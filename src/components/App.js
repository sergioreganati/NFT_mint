...
import looksrare from '../images/socials/Looksrare.svg'
...
function App() {
	...
	const [looksrareURL, setLooksRareURL] = useState('https://goerli.looksrare.org')
	...
	const loadWeb3 = async () => {
		if (typeof window.ethereum !== 'undefined') {
			...
			if (networkId !== 5777 && networkId !== 'undefined') {
				setExplorerURL(config.NETWORKS[networkId].explorerURL)
				setLooksRareURL(config.NETWORKS[networkId].looksrareURL)
			}
				...
				<section id='welcome' className='welcome'>
					...
					<Row className='header my-3 p-3 mb-0 pb-0'>
						...
						<Col className='flex social-icons'>
							...
							<a
								href={`${looksrareURL}/collections/${config.PROJECT_ADDRESS}`}
								target='_blank'
								className='circle flex button'>
					...
					<Row className='flex m-3'>
						...
						<Col md={5} lg={4} xl={5} xxl={4}>
							...
							<p className='text'>
								Developed while attending Blockchain Mastery University, learned how to generate NFT images, upload to IPFS, create your NFT contract, and use LooksRare for test networks! Check it out!
							</p>
						...
						<Col md={5} lg={4} xl={5} xxl={4}>
							{isError ? (
								...
								<div>
									...
									<ul>
										<li>100 generated punked out images using an art generator</li>
										<li>Free minting on Goerli testnet</li>
										<li>Viewable on LooksRare shortly after minting</li>
									</ul>
									...
									{ownerOf.length > 0 &&
										<p><small>View your NFT on
											<a
												href={`${looksrareURL}/collections/${openPunks._address}/${ownerOf[0]}`}
												target='_blank'
